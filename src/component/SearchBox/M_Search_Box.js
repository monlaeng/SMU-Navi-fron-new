    ...
    function infoDetail(index) {
        ...
        if(showInfo[index] === false) {
            return (
                <>
                    {progress(index)}
                    <span><img id={'arrow2'} src={arrow}/></span>
            ...
            return (
                <div>
                    {transferName[index].map((obj, index2) => (
                        <div key={index2}>
                            <div id={"wayDetail"}>
                                <p id={'takePoint2'}> <span id={'firstRoute2'} style={{backgroundColor: colorSelector(ways[index].subPathList[index2].transitType,ways[index].subPathList[index2].busType,ways[index].subPathList[index2].lineName)}}>{lineName(index, index2)}</span>{obj.from}</p>
                                <h6 id={"detailMin2"}>{obj.sectionTime}분</h6>
                                <div id={"infoDetail"}>
                    ...
                    <span><img id={'finalIcon2'} src={finalIcon}/><p id={'finalPlace2'}><span id={'finish'}>도착</span> 상명대 도착</p></span>
                </div>
            )
        }
    }
            ...
            <div id={"dropDown"}>
                {position.map((obj, index) => (
                    <ol id={'list'} key={index}>
                        <p id={"locBtn"+index} onClick={e => {buttonSelect(e, index); setView(!view);}}>{obj.title}</p>
                            </ol>
                            ))}
                    </div>
                )
